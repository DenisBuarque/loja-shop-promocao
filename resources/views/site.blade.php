...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body>
                ...
                <div class="ms:container ms:mx-auto flex flex-col items-center justify-end">
                    ...
                    <div class="flex flex-wrap md:justify-evenly w-full">
                        @foreach ($categories as $category)
                            <a href="{{route('category',$category->slug)}}" class="border border-white text-white px-5 py-3 items-center justify-center m-1 rounded-full hover:bg-white hover:text-black">{{ $category->name }}</a>
                        @endforeach
                    </div>
                    <div class="w-full pt-5">
                        <form method="POST" action="{{route('search')}}" class="flex flex-col md:flex-row">
                            @csrf
                            <input 
                                type="search" 
                                name="search" 
        ...
        <aside class="bg-gray-100 w-full px-2 py-10">

            @if (session('alert'))
                <div class="bg-red-600 text-white p-3 rounded-lg container m-auto">
                    {{ session('alert') }}
                </div>
                <br/><br/>
            @endif

            <section class="container mx-auto grid gap-4 grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
